...
//    }
        two_three_tree ttt = new upgraded_two_three_tree(Integer.MAX_VALUE, Integer.MIN_VALUE);
        ttt.two_three_init();
        Map<Integer, String> faculties = new HashMap<>();
        faculties.put(1, "CS");
        faculties.put(2, "EE");
        ...
        for (Map.Entry<Integer, String> f : faculties.entrySet()) {
            Faculty faculty = new Faculty(f.getKey(), f.getValue());
            ttt.insert(new Node<Integer, Integer>(faculty.getId(), faculty.getId()));
        }
        for (Map.Entry<Integer, String> f : faculties.entrySet()) {
            Node node = ttt.two_three_search(ttt.getRoot(), f.getKey());
            System.out.println(node.getKey() + " predecessor is " + node.getPredecessor().getKey());
        }
    }
